...
import com.pixelsface.snake.AppConstants.Control;
import com.pixelsface.snake.AppConstants.Direction;
import com.pixelsface.snake.SnakeHelper.DrawControl;
import com.pixelsface.snake.SnakeHelper.SnakeMovement;
import com.pixelsface.snake.SnakeHelper.Utils;
...
public class SnakeView extends SurfaceView implements Runnable {
    ...
    private final long MILLIS_IN_A_SECOND = 3000;
    ...
    public SnakeView(Context context, Point size) {
        ...
        mCustomImage = context.getResources().getDrawable(R.drawable.apple);
        bitmap = Bitmap.createBitmap(mCustomImage.getIntrinsicWidth(), mCustomImage.getIntrinsicHeight(), Bitmap.Config.ARGB_8888);
//        bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.apple);
    ...
    public void loadSound() {
        ...
        try {
            ...
            descriptor = assetManager.openFd("eat.ogg");
            m_get_mouse_sound = m_SoundPool.load(descriptor, 0);

            descriptor = assetManager.openFd("collide.ogg");
    ...
    private void eatMouse() {
        ...
        m_Score = m_Score + 1;
        if (isSoundEnabled)
            m_SoundPool.play(m_get_mouse_sound, 1, 1, 0, 0, 0.5f);
    ...
    public void updateGame() {
        ...
        if (detectDeath()) {
            ...
            ((com.pixelsface.snake.SnakeActivity) m_context).finish();
...
//            m_Canvas.drawCircle((m_MouseX * m_BlockSize) + size, (m_MouseY * m_BlockSize) + size, size, m_Paint);
            ...
            mCustomImage.setBounds(0,0 , canvas.getWidth(), canvas.getHeight());
            mCustomImage.draw(canvas);
            m_Canvas.drawBitmap(bitmap, (m_MouseX * m_BlockSize) , (m_MouseY * m_BlockSize) , m_Paint);


            // Set the color of the paint to draw the snake with
//            m_Paint.setColor(Color.argb(255,  120, 197, 87));
            m_Paint.setColor(Color.argb(255, 0, 0, 0));
            //Draw the snake
            ...
            m_Canvas.drawText("Score:" + m_Score, 10, 100, m_Paint);

            int highScore = preferences.getInt("HighScore", 0);
            m_Canvas.drawText("High Score:" + highScore, m_ScreenWidth - Utils.pxFromDp(m_context, 150), 100, m_Paint);
